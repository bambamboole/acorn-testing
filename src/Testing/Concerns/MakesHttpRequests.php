...
/**
 * Issue HTTP requests against the booted application from a feature test.
 *
 * Laravel ships this in illuminate/foundation, which Acorn projects do not have
 * — Acorn pulls individual illuminate/* components. Vendored here so consumers
 * do not each copy it. The companion TestResponse is *not* vendored:
 * illuminate/testing provides it, and a local snapshot only goes stale.
 *
 * Requires the host class to expose a container as $this->app.
 */
